...
[技法地图](editing-techniques.md)指导助手选择剪法；[执行说明](execution-recipes.md)区分现有脚本与需另行实现的处理。当前 EDL 没有 transition、speed、broll、ducking、独立声画入出点等字段；未知字段可能被忽略。请勿以新增字段代替真正实现。进阶决定使用独立的剪辑决策表，完成相应本地工程后再标为已执行。
